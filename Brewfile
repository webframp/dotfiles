tap "2mol/tools"
tap "chef/chef"
tap "chrokh/tap"
tap "crisidev/chunkwm"
tap "d12frosted/emacs-plus"
tap "derailed/k9s"
tap "getantibody/tap"
tap "golangci/tap"
tap "habitat-sh/habitat"
tap "homebrew/bundle"
tap "homebrew/cask"
tap "homebrew/cask-fonts"
tap "homebrew/core"
tap "homebrew/services"
tap "instrumenta/instrumenta"
tap "jmacdonald/amp"
tap "joemiller/taps"
tap "khanhas/tap"
tap "koekeishiya/formulae"
tap "osx-cross/avr"
tap "px4/px4"
tap "twpayne/taps"
tap "versent/taps"
tap "weaveworks/tap"
tap "webframp/osxsec"
tap "webframp/tap"
tap "zaquestion/tap"
brew "antiword"
brew "archey"
brew "aspell"
brew "autoconf"
brew "automake"
brew "avrdude", args: ["HEAD", "with-libhid"]
brew "aws-iam-authenticator"
brew "gdbm"
brew "openssl@1.1"
brew "readline"
brew "python"
brew "awscli"
brew "bat"
brew "bazelisk"
brew "binwalk"
brew "bison"
brew "black"
brew "libffi"
brew "chruby"
brew "cocoapods"
brew "consul"
brew "consul-template"
brew "cookiecutter"
brew "coreutils"
brew "cowsay"
brew "csvkit"
brew "dash"
brew "davmail"
brew "go"
brew "dep"
brew "dependency-check"
brew "dfu-programmer"
brew "dfu-util"
brew "direnv"
brew "dos2unix"
brew "editorconfig"
brew "envchain"
brew "exa"
brew "fd"
brew "gnutls"
brew "ffmpeg"
brew "flake8"
brew "fluxctl"
brew "fpp"
brew "fzf"
brew "git"
brew "git-lfs"
brew "git-secrets"
brew "gitversion"
brew "glances"
brew "gnu-sed"
brew "gnu-tar"
brew "gnupg"
brew "graphviz"
brew "hadolint"
brew "haskell-stack"
brew "helm"
brew "ncurses"
brew "htop"
brew "hub"
brew "hugo"
brew "libtool"
brew "ispell"
brew "jansson"
brew "jfrog-cli-go"
brew "jq"
brew "node"
brew "jsonlint"
brew "kubernetes-cli"
brew "kops"
brew "kubectx"
brew "kustomize"
brew "languagetool"
brew "lastpass-cli"
brew "libressl"
brew "libyaml"
brew "make"
brew "mas"
brew "mkcert"
brew "mono"
brew "mr"
brew "mysql-client"
brew "mysql@5.6"
brew "nikto"
brew "nmap"
brew "octant"
brew "opa"
brew "packer"
brew "pandoc"
brew "pass"
brew "pinentry-mac"
brew "pipenv"
brew "poppler"
brew "proselint"
brew "pstree"
brew "pulumi"
brew "pyenv"
brew "pyenv-virtualenv"
brew "r"
brew "reattach-to-user-namespace"
brew "ripgrep"
brew "ruby-install"
brew "rust"
brew "rustup-init"
brew "s3cmd"
brew "shellcheck"
brew "ship"
brew "source-highlight"
brew "spark"
brew "starship"
brew "swiftlint"
brew "terraform"
brew "texinfo"
brew "the_silver_searcher"
brew "tldr"
brew "tmux"
brew "travis"
brew "unoconv"
brew "urlview"
brew "vault"
brew "vdirsyncer"
brew "volatility"
brew "watch"
brew "watchman"
brew "wget"
brew "wireguard-tools"
brew "xmlformat"
brew "yamllint"
brew "youtube-dl"
brew "z"
brew "zsh"
brew "zsh-completions"
brew "2mol/tools/pboy"
brew "d12frosted/emacs-plus/emacs-plus", args: ["HEAD", "with-jansson", "with-modern-icon", "with-no-titlebar", "with-window-role-fix", "without-imagemagick@7", "without-spacemacs-icon"]
brew "derailed/k9s/k9s"
brew "getantibody/tap/antibody"
brew "golangci/tap/golangci-lint"
brew "habitat-sh/habitat/hab"
brew "instrumenta/instrumenta/conftest"
brew "joemiller/taps/vault-token-helper"
brew "khanhas/tap/spicetify-cli"
brew "koekeishiya/formulae/skhd"
brew "koekeishiya/formulae/yabai"
brew "osx-cross/avr/avr-gcc@7", link: true
brew "px4/px4/gcc-arm-none-eabi"
brew "twpayne/taps/chezmoi"
brew "versent/taps/saml2aws"
brew "weaveworks/tap/eksctl"
brew "webframp/tap/bandwhich"
brew "webframp/tap/cistern"
brew "webframp/tap/ec2instanceconnect"
brew "zaquestion/tap/lab"
cask "adobe-digital-editions"
cask "aerial"
cask "alfred"
cask "avira-antivirus"
cask "backblaze"
cask "balenaetcher"
cask "bitbar"
cask "blockblock"
cask "bloodhound"
cask "calibre"
cask "chef-workstation"
cask "dhs"
cask "docker"
cask "dotnet-sdk"
cask "dropbox"
cask "edex-ui"
cask "etcher"
cask "fastlane"
cask "flux"
cask "font-fira-code"
cask "font-firacode-nerd-font-mono"
cask "font-hack"
cask "font-hack-nerd-font"
cask "font-meslo-for-powerline"
cask "font-noto-sans"
cask "font-open-sans"
cask "font-roboto"
cask "font-source-code-pro"
cask "font-sourcecodepro-nerd-font"
cask "grandperspective"
cask "handbrake"
cask "java"
cask "keybase"
cask "keycastr"
cask "knockknock"
cask "krisp"
cask "lockrattler"
cask "lunchy"
cask "mactex"
cask "microsoft-azure-storage-explorer"
cask "microsoft-teams"
cask "mysqlworkbench"
cask "neo4j"
cask "ngrok"
cask "overkill"
cask "oversight"
cask "powershell"
cask "ransomwhere"
cask "rq"
cask "sekey"
cask "signal"
cask "skype-for-business"
cask "slack"
cask "sonic-pi"
cask "spotify"
cask "steam"
cask "systhist"
cask "taskexplorer"
cask "timer"
cask "uxprotect"
cask "vagrant"
cask "virtualbox"
cask "virtualbox-extension-pack"
cask "viscosity"
cask "visual-studio-code"
cask "vivaldi"
cask "vlc"
cask "vmware-horizon-client"
cask "whatsapp"
cask "zoomus"
mas "DXF Viewer Pro", id: 1093799308
mas "GarageBand", id: 682658836
mas "GIF Keyboard", id: 1043270657
mas "iMovie", id: 408981434
mas "Keynote", id: 409183694
mas "Kindle", id: 405399194
mas "LastPass", id: 926036361
mas "LightBlue", id: 639944780
mas "Microsoft OneNote", id: 784801555
mas "Microsoft Remote Desktop", id: 1295203466
mas "MindNode", id: 1289197285
mas "NetSpot", id: 514951692
mas "Numbers", id: 409203825
mas "Pages", id: 409201541
mas "Swiftify for Xcode", id: 1183412116
mas "Trello", id: 1278508951
mas "Tweetbot", id: 557168941
mas "WireGuard", id: 1451685025
mas "Xcode", id: 497799835
